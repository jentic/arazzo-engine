...
from arazzo_runner.auth.credentials.provider import CredentialProviderFactory
...
def test_resolve_credentials_api_key(api_key_req, env_mappings, monkeypatch):
    ...
    provider = CredentialProviderFactory.create_default(
        env_mapping=env_mappings, http_client=MagicMock(), auth_requirements=[api_key_req]
    )

    # Create security option with API Key requirement
    security_option = SecurityOption(
        requirements=[SecurityRequirement(scheme_name="ApiKeyAuth", scopes=[])]
    )
...
def test_resolve_credentials_bearer(env_mappings, bearer_req, monkeypatch):
    ...
    provider = CredentialProviderFactory.create_default(
        env_mapping=env_mappings, http_client=MagicMock(), auth_requirements=[bearer_req]
    )

    # Create security option with Bearer requirement
    security_option = SecurityOption(
        requirements=[SecurityRequirement(scheme_name="BearerAuth", scopes=[])]
    )
...
def test_resolve_credentials_basic(env_mappings, basic_req, monkeypatch):
    ...
    provider = CredentialProviderFactory.create_default(
        env_mapping=env_mappings, http_client=MagicMock(), auth_requirements=[basic_req]
    )

    # Create security option with Basic auth requirement
    security_option = SecurityOption(
        requirements=[SecurityRequirement(scheme_name="BasicAuth", scopes=[])]
    )
...
def test_resolve_credentials_multiple_options(
    env_mappings, api_key_req, bearer_req, basic_req, monkeypatch
):
...
def test_resolve_credentials_combined_requirements(
    env_mappings, api_key_req, bearer_req, monkeypatch
):
...
def test_resolve_credentials_with_source_name(env_mappings, monkeypatch):
    ...
    env_mappings = {
        "source1": {"ApiKeyAuth": {EnvVarKeys.API_KEY: "TEST_API_KEY_SOURCE1"}},
        "source2": {"ApiKeyAuth": {EnvVarKeys.API_KEY: "TEST_API_KEY_SOURCE2"}},
    }
...
def test_resolve_credentials_oauth2_client_credentials(monkeypatch, env_mappings):
    ...
    oauth2_req = {
        ...
        "auth_urls": {"token": "https://example.com/token"},
    }
    ...
    provider = CredentialProviderFactory.create_default(
        env_mapping=env_mappings, http_client=mock_http_client, auth_requirements=[oauth2_req]
    )

    # Create security option with OAuth2 requirement
    security_option = SecurityOption(
        requirements=[SecurityRequirement(scheme_name="OAuth2Auth", scopes=["read", "write"])]
    )
