...
MOCK_OP_DETAILS_BODY_REQUIRED = {
    ...
    "operation": {"summary": "Create item", "operationId": "createItem", "parameters": []},
    "requestBody": {
        "required": True,
        "content": {
            "application/json": {
...
MOCK_OP_DETAILS_BODY_REF = {
    ...
    "operation": {
        ...
        "requestBody": {"$ref": "#/components/requestBodies/UserBody"},  # Reference
    },
}
...
class TestParameterProcessorOperation(unittest.TestCase):
    ...
    def test_prepare_params_required_missing_error(self):
        ...
        with self.assertRaisesRegex(
            ValueError, r"Required parameter 'itemId' \(in: path\) is missing."
        ):
    ...
    def test_prepare_body_multi_content_json_priority(self):
        ...
        inputs = {"key": "value"}
        result = self.processor.prepare_operation_parameters(
            MOCK_OP_DETAILS_BODY_MULTI_CONTENT_JSON_FIRST, inputs
        )
        expected_body = {
            "payload": {"key": "value"},
    ...
    def test_prepare_body_multi_content_first_priority(self):
        ...
        result = self.processor.prepare_operation_parameters(
            MOCK_OP_DETAILS_BODY_MULTI_CONTENT_NO_JSON, inputs
        )
        expected_body = {
    ...
    def test_prepare_body_with_ref(self):
        ...
        self.processor._resolve_ref = MagicMock(
            return_value=MOCK_COMPONENTS["requestBodies"]["UserBody"]
        )
    ...
    def test_path_param_from_path_string_only(self):
        ...
        with self.assertRaisesRegex(
            ValueError, r"Required parameter 'widget_id' \(in: path\) is missing."
        ):
