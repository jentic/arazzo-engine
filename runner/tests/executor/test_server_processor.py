...
from arazzo_runner.models import RuntimeParams, ServerConfiguration, ServerVariable
...
def test_resolve_with_runtime_param(mock_env):
    ...
    resolved_url = ServerProcessor.resolve_server_base_url(
        config, server_runtime_params={"RUNNER_SERVER_REGION": "us-west"}
    )
...
def test_resolve_with_env_var_no_prefix(mock_env):
    ...
    config = _create_server_config(
        "https://{endpoint}.example.com", variables={"endpoint": sv_endpoint}
    )
...
def test_resolve_with_env_var_with_prefix(mock_env):
    ...
    config = _create_server_config(
        "https://api.{instance}.com", variables={"instance": sv_instance}, api_title_prefix="MYAPI"
    )
    resolved_url = ServerProcessor.resolve_server_base_url(config)
...
def test_resolve_precedence_runtime_over_env_over_default(mock_env):
    ...
    config = _create_server_config("{host}/data", variables={"host": sv_host})
    resolved_url = ServerProcessor.resolve_server_base_url(
        config, server_runtime_params={"RUNNER_SERVER_HOST": "runtime-host"}
    )
...
def test_resolve_precedence_runtime_over_default(mock_env):
    ...
    resolved_url = ServerProcessor.resolve_server_base_url(
        config, server_runtime_params={"RUNNER_SERVER_PATH": "runtime_path"}
    )
...
def test_resolve_missing_required_variable_with_prefix_raises_value_error(mock_env):
    ...
    config = _create_server_config(
        "/{user}/profile", variables={"user": sv_user}, api_title_prefix="PORTAL"
    )
    with pytest.raises(ValueError):
        ServerProcessor.resolve_server_base_url(config)
...
def test_resolve_multiple_variables(mock_env):
    ...
    config = _create_server_config(
        ...
        variables={"scheme": sv_scheme, "host": sv_host, "port": sv_port, "basepath": sv_basepath},
    )
    resolved_url = ServerProcessor.resolve_server_base_url(
        config,
        server_runtime_params={
            "RUNNER_SERVER_HOST": "localhost",
            "RUNNER_SERVER_BASEPATH": "custom_api",
        },
    )
...
def test_resolve_server_params_with_variables():
    sv_endpoint = _create_server_variable(name="endpoint")
    config = _create_server_config(
        "https://api.{endpoint}.example.com/v1/users", variables={"endpoint": sv_endpoint}
    )
...
def test_resolve_server_params_with_double_variables():
    sv_endpoint = _create_server_variable(name="endpoint")
    config = _create_server_config(
        "https://api.{endpoint}.{endpoint}.example.com/v1/users",
        variables={"endpoint": sv_endpoint},
    )
    runtime_params = RuntimeParams(servers={"endpoint": "test"})
    final_url = ServerProcessor.resolve_server_base_url(
        server_config=config,
