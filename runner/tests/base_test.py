#!/usr/bin/env python3
"""
Base Test Utilities for Arazzo Runner
...
class ArazzoTestCase(unittest.TestCase):
    """
    Base test case for Arazzo Runner tests
    ...
    def create_arazzo_runner(
        self, arazzo_doc: dict[str, Any], source_descriptions: dict[str, Any]
    ) -> ArazzoRunner:
        """
        Create an Arazzo Runner instance for a specific workflow
        ...
        Returns:
            Configured ArazzoRunner instance
        """
        print("SOOURCEDESC", source_descriptions)
        return ArazzoRunner(arazzo_doc, source_descriptions, http_client=self.http_client)
